...
def preprocess_data(df):
    ...
    # Convert columns with currency symbols or commas to numeric if possible (skip Date and Text columns)
    for col in df.columns:
        if df[col].dtype == 'object' and col not in ['Date', 'Text']:
            try:
                ...
                continue
    # Encode categorical data (do not process Date or Text columns)
...
def feature_selection(df, target):
    if target in df.columns:
        ...
        model.fit(X)
        # Wrap the predict function so SHAP can call it
...
def train_models(df, target):
    ...
    results = {
        ...
        "XGBoost Regression MSE": xg_mse,
    }
    return results, (lr, xg_reg), (X_test, y_test, lr_pred, xg_pred)
...
if uploaded_file:
    ...
    if option_regression:
        results, models, (X_test, y_test, lr_pred, xg_pred) = train_models(df, target_col)
        st.subheader("Regression Analysis (Train-Test Split)")
        st.write("Regression Results:", results)
        fig_lr = px.scatter(x=y_test, y=lr_pred, labels={'x':"Actual", 'y':"Predicted"}, title="Linear Regression Predictions")
        st.plotly_chart(fig_lr)
        fig_xgb = px.scatter(x=y_test, y=xg_pred, labels={'x':"Actual", 'y':"Predicted"}, title="XGBoost Regression Predictions")
    ...
    if option_model_performance:
        ...
        results, _, (X_test, y_test, lr_pred, xg_pred) = train_models(df, target_col)
        st.write("Linear Regression MSE:", results["Linear Regression MSE"])
        st.write("XGBoost Regression MSE:", results["XGBoost Regression MSE"])
        fig_lr = px.scatter(x=y_test, y=lr_pred, labels={'x':"Actual", 'y':"Predicted"}, title="Linear Regression Predictions")
        st.plotly_chart(fig_lr)
